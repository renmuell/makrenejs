...
var _hammer = require('../vendors/hammer.min');

var _hammer2 = _interopRequireDefault(_hammer);

require('../vendors/hammer-time.min');
  ...
  var inputPlugin = {
    ...
    create: function create(config) {
      ...
      var input = {
        ...
        setupHammer: function setupHammer() {
          input.hammerManager = new _hammer2.default.Manager(input.engine.domElement);

          input.hammerManager.add(new _hammer2.default.Tap({
            event: 'singleTap'
          }));

          input.hammerManager.add(new _hammer2.default.Pan({
            event: 'pan'
          }));

          input.hammerManager.get('singleTap').recognizeWith('pan');

          input.hammerManager.on('singleTap', function (ev) {
            input.gestures.singleTap = ev.center;
          });

          input.hammerManager.on('pan', function (ev) {
        ...
        postStep: function postStep() {
          if (input.gestures.singleTap) {
            input.onTap(input.getCanvasPosition(input.gestures.singleTap));

            input.gestures.singleTap = undefined;
          }

          if (input.gestures.pan) {
            input.onPan(input.getCanvasPosition(input.gestures.pan));
        ...
        getCanvasPosition: function getCanvasPosition(event) {
          ...
          event.x = event.x - rect.left, event.y = event.y - rect.top;
...
},{"../vendors/hammer-time.min":2,"../vendors/hammer.min":3}],2:[function(require,module,exports){
